...
<center>
                ...
                <!-- Range Code -->
                       ...
                       <td style="background-color:grey;color:black;font-size:15px;" class="center aligned" > <para>Mapping with CO</para></td>
                        <?php
                            ...
                            {
                                if($n[$i]==1){ ?>
                            <td style="background-color:#dfedf7;color:black;font-size:15px;" class="center aligned" > <?php echo "CO".$i; ?></td>
                               <?php } }?>
    ...
    if(isset($_POST["submit"]))
    {
        ...
        $maxmark = $max->fetch_row();
        //echo $maxmark[0];
        $sql="SELECT * from $_table WHERE `sec` like '".strtolower($class)."' ORDER BY `rollno` ASC ";
        $data1=$con->query($sql);
        $tot_num=array(0,0,0,0,0);
        $got=array(0,0,0,0,0);
        $exp_att_arr=array(0,0,0,0,0); 
        $sat_att_arr=array(0,0,0,0,0); 
        $Up2ExpLvl=array(0,0,0,0,0);
        $AttLvlCo=array(0,0,0,0,0);
        $range=array();
        //print_r($range);
        //echo $sql;
        ...
        for($i=0;$i<5;$i++){
            if($tot_num[$i]!=0){
                $Up2ExpLvl[$i]=number_format((($got[$i]/$tot_num[$i])*100),2);
                $exp_att_arr[$i]=number_format((($expmark[$i+2]/$maxmark[$i+2])*100),2);
            }
        }
        
        for($i=0;$i<5;$i++){
            $temp=number_format((($exp_att_arr[$i]-50)/3),2);
            $temp_arr=array();
            array_push($temp_arr,50,50+$temp,$exp_att_arr[$i]-$temp,$exp_att_arr[$i]);
            $range[$i]=$temp_arr;
            $temp_rem=($exp_att_arr[$i]-50)-($temp*3);
            if($temp_rem==2)
            {
            $range[$i][1]+=1;
            $range[$i][3]-=1;
        ...
        {
        if($tot_num[$i]!=0){
        if($exp_att_arr[$i]<=$range[$i][0])
        {
            $sat_att_arr[$i]=1;
        }
        else if($exp_att_arr[$i]>$range[$i][0] && $exp_att_arr[$i]<=$range[$i][1])
        {
            $sat_att_arr[$i]=2;
        }
        else if($exp_att_arr[$i]>$range[$i][1] && $exp_att_arr[$i]<=$range[$i][2])
        {
            $sat_att_arr[$i]=3;
        }
        else if($exp_att_arr[$i]>$range[$i][2] && $exp_att_arr[$i]<=$range[$i][3])
        {
            $sat_att_arr[$i]=4;
        }
        else
        ...
        {
        if($tot_num[$i]!=0){
            if($Up2ExpLvl[$i]<=$range[$i][0])
            {
                $AttLvlCo[$i]=1;
            }
            else if($Up2ExpLvl[$i]>$range[$i][0] && $Up2ExpLvl[$i]<=$range[$i][1])
            {
                $AttLvlCo[$i]=2;
            }
            else if($Up2ExpLvl[$i]>$range[$i][1] && $Up2ExpLvl[$i]<=$range[$i][2])
            {
                $AttLvlCo[$i]=3;
            }
            else if($Up2ExpLvl[$i]>$range[$i][2] && $Up2ExpLvl[$i]<=$range[$i][3])
            {
                $AttLvlCo[$i]=4;
            }
            else
